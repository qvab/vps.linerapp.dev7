...
class TestController extends Controller
{
  ...
  public function test()
  {
    ...
    for ($i = 0; $i < 1000; $i++) {
      $arUser = $this->distr();
      if (!empty($arTest[$arUser["name"]])) {
        $arTest[$arUser["name"]]["Количество_заявок"]++;
      } else {
        $arTest[$arUser["name"]] = [
          "Процент" => $arUser["rate"],
          "Количество_заявок" => 0,
    ...
    ?><pre><?php print_r($arTest); ?></pre><?php
  }
  ...
  public function distr()
  {
    ...
    include_once $_SERVER["DOCUMENT_ROOT"]."/miwr/classes/class.distrbution.php";
    $mwDist = new \MIWR\DistributionLog();
    $currentPipeline = 1435261; //$request->input('leads.add.0.pipeline_id');                 // ID воронки
    $iTotalBall = 100000;
    $arError = [
      "initSuccess" => 0,
    ...
    $account = Account::where("subdomain", "linerappwidgets")->with("distribution")->first();
        ...
        if (!empty($account->distribution->fields)) {                                       // Проверка наличия полей для распределение заявок
          ...
          if (!empty($arPipeline["fields"])) {
            ...
            foreach ($arPipeline["fields"] as $iUserId => $arUser) {
              ...
              if (!empty($bCalcExpTask) && !empty($arPipeline["considerationExpTasks"])) {
              ...
              if (!empty($arPipeline["considerationExpTasks"])) {
                $arPipelineUsers[$iPipelineId]["fields"][$iUserId]["expTasks"] = $iExpTask[$iUserId];
                $arPipelineUsers[$iPipelineId]["fields"][$iUserId]["totalTasks"] = $iTotalTasks[$iUserId];
              }
  ...
  public
  function getAllAutoTask(Request $request, Account $account)
  {
    return response()->json([
  ...
  public
  function getAutoTaskById(Request $request, Account $account, AutoTask $autotask)
  {
